...
- ✅ Companies API (full CRUD + bulk import + optimized endpoint + export)
- ✅ Contacts API (full CRUD + bulk import)
- ✅ Markets API (full CRUD + bulk import)
- ✅ Verticals API (full CRUD + bulk import)
- ✅ Digital Snapshots API (full CRUD + bulk import)
...
- ✅ Platform Users API (admin user management)
- ✅ Audit Logs API (read-only)
- ✅ Health Check API (system monitoring)
- ✅ Performance Monitoring API (metrics and optimization)
- ✅ Security Reporting API (incident reporting)
- ✅ Import Counts API (data import statistics)
...
**HEAD Request for Simple Uptime:**
```http
HEAD /api/health
```
...
**Query Parameters:**
- `metric` (optional) - Specific metric: `queries`, `cache`, `slow-queries`, `analysis`, `system`
- `timeRange` (optional) - Time range for metrics (default: `1h`)
- `limit` (optional) - Limit for slow queries (default: 20)
...
```json
{
  "action": "clear-cache" | "warm-cache" | "optimize-queries",
  "params": {
    "organizationId": "uuid" // Required for warm-cache
  }
}
```
...
**High-Performance Companies Endpoint:**
```http
GET /api/companies-optimized?organization_id=uuid
```
...
**Query Parameters:**
- Same as regular companies API
- Additional `limit` and `offset` for pagination
...
```http
OPTIONS /api/companies-optimized?action=performance
OPTIONS /api/companies-optimized?action=cache-status
```
...
```json
{
  "type": "csrf_violation" | "rate_limit_exceeded" | "suspicious_activity",
  "details": {
    "ip": "192.168.1.1",
