...
export async function PATCH(
  ...
  try {
    ...
    if (oldData) await logUpdate({ supabase, request }, 'result', id, oldData, data)
...
export async function DELETE(
  ...
  try {
    ...
    const userOrgId = await resolveUserOrgId(supabase)
    if (!userOrgId || !resultData || resultData.organization_id !== userOrgId) {
      return NextResponse.json({ error: 'Forbidden' }, { status: 403 })
    }
    ...
    if (resultData) {
      await logDelete({ supabase, request }, 'result', resultData)
    }
